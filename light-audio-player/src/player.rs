use std::{env, vec::Vec, thread, io, 
    ...
    fs::{File, read_dir}, io::Write};
use rand::{seq::SliceRandom, thread_rng};
use rodio::{Decoder, OutputStream, Sink};

// ---------------------- Constants ---------------------
const VOLUME_ADJUSTMENT: f32 = 0.1f32;
const KEEP_ALIVE_TIME: u64 = 1;
#[derive(Debug)]
struct Message {
    contents: Contents,
    origin: Origin,
...
fn input_thread(to: Sender<Message>, rcv: Receiver<usize>) -> Result<(), io::Error> {
    ...
    loop {
        ...
        println!("{}", buf);
        match buf.as_str() {
            "p\n" => to.send(Message::new(Contents::Play, Origin::Input)).unwrap(),
            "=\n" => to.send(Message::new(Contents::Pause, Origin::Input)).unwrap(), 
            "#\n" => to.send(Message::new(Contents::Stop, Origin::Input)).unwrap(), 
            "s\n" => to.send(Message::new(Contents::Skip, Origin::Input)).unwrap(), 
